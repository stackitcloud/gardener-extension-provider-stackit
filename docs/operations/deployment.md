...
**Disclaimer:** This document is NOT a step by step installation guide for the STACKIT provider extension and only contains some configuration specifics regarding the installation of different components via the helm charts residing in the STACKIT provider extension [repository](https://github.com/stackit/gardener-extension-provider-stackit).
...
The easiest way to deploy the `gardener-extension-admission-stackit` component will be to not provide `kubeconfig` at all. This way in-cluster configuration and an automounted service account token will be used. The drawback of this approach is that the automounted token will not be automatically rotated.
