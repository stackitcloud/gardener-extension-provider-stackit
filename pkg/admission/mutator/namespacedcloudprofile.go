...
func (p *namespacedCloudProfile) mutateSpecCapabilityFlavors(ctx context.Context, profile *gardencorev1beta1.NamespacedCloudProfile, specConfig *stackitv1alpha1.CloudProfileConfig) error {
...
func (p *namespacedCloudProfile) mergeStatusProviderConfig(profile *gardencorev1beta1.NamespacedCloudProfile, specConfig *stackitv1alpha1.CloudProfileConfig) error {
	statusConfig := &stackitv1alpha1.CloudProfileConfig{}
	...
	// Overwrite API endpoints from spec
	if specConfig.APIEndpoints != nil {
...
func mergeMachineImages(specMachineImages, statusMachineImages []stackitv1alpha1.MachineImages) []stackitv1alpha1.MachineImages {
	specImages := utils.CreateMapFromSlice(specMachineImages, func(mi stackitv1alpha1.MachineImages) string { return mi.Name })
	statusImages := utils.CreateMapFromSlice(statusMachineImages, func(mi stackitv1alpha1.MachineImages) string { return mi.Name })
	for _, specMachineImage := range specImages {
		if _, exists := statusImages[specMachineImage.Name]; !exists {
			...
			statusImageVersions := utils.CreateMapFromSlice(statusImages[specMachineImage.Name].Versions, func(v stackitv1alpha1.MachineImageVersion) string { return v.Version })
			specImageVersions := utils.CreateMapFromSlice(specImages[specMachineImage.Name].Versions, func(v stackitv1alpha1.MachineImageVersion) string { return v.Version })
			for _, version := range specImageVersions {
				statusImageVersions[version.Version] = version
			}

			statusImages[specMachineImage.Name] = stackitv1alpha1.MachineImages{
