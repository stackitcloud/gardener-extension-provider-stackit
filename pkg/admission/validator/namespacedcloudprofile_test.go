...
"keystoneURL":"http://url-to-keystone/v3"
			...
			Expect(err).To(ConsistOf(PointTo(MatchFields(IgnoreExtras, Fields{
				...
				"Detail": Equal("must only set machineImages or apiEndpoints"),
			}))))
		})

		It("should succeed for NamespacedCloudProfile specifying only apiEndpoints", func() {
			namespacedCloudProfile.Spec.ProviderConfig = &runtime.RawExtension{Raw: []byte(`{
"apiVersion":"stackit.provider.extensions.gardener.cloud/v1alpha1",
"kind":"CloudProfileConfig",
...
"machineImages":[
  {"name":"image-1","versions":[{"version":"1.0","image":"image-name-1","regions":[{"name":"image-region-1","id":"id-img-reg-1"}]}]}
			...
			namespacedCloudProfile.Spec.MachineImages = []core.MachineImage{
				{
					Name:     "image-1",
					Versions: []core.MachineImageVersion{{ExpirableVersion: core.ExpirableVersion{Version: "1.0"}, Architectures: []string{"amd64"}}},
				},
			}
