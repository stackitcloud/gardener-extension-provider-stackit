...
func (a *networkingAccess) GetRouterByID(ctx context.Context, id string) (*Router, error) {
	allRouters, err := a.networking.ListRouters(ctx, routers.ListOpts{ID: id})
	if err != nil {
		return nil, err
	}
	if len(allRouters) == 0 {
	...
	return a.toRouter(&allRouters[0]), nil
}
...
func (a *networkingAccess) GetRouterByName(ctx context.Context, name string) ([]*Router, error) {
	allRouters, err := a.networking.ListRouters(ctx, routers.ListOpts{Name: name})
	if err != nil {
		return nil, err
	}
	result := make([]*Router, 0, len(allRouters))
	for _, raw := range allRouters {
		...
		case <-ticker:
			...
			case "BUILD", "PENDING_CREATE", "PENDING_UPDATE", "DOWN":
				...
				if a.log.V(1).Enabled() {
					...
					a.log.V(1).Info("port info", "Port", string(marshaled))
				}
			case "ACTIVE":
				return nil
			default:
...
func (a *networkingAccess) GetNetworkByID(ctx context.Context, id string) (*Network, error) {
	allNetworks, err := a.networking.ListNetwork(ctx, networks.ListOpts{ID: id})
	if err != nil {
		return nil, err
	}
	...
	return a.toNetwork(&allNetworks[0]), nil
}
...
func (a *networkingAccess) GetNetworkByName(ctx context.Context, name string) ([]*Network, error) {
	allNetworks, err := a.networking.GetNetworkByName(ctx, name)
	if err != nil {
		return nil, err
	}
	result := make([]*Network, 0, len(allNetworks))
	for _, raw := range allNetworks {
...
func (a *networkingAccess) GetSubnetByName(ctx context.Context, networkID, name string) ([]*subnets.Subnet, error) {
	...
	result := make([]*subnets.Subnet, 0, len(list))
...
func (a *networkingAccess) GetSecurityGroupByName(ctx context.Context, name string) ([]*groups.SecGroup, error) {
	...
	result := make([]*groups.SecGroup, 0, len(list))
	for _, raw := range list {
		tmp := raw
		result = append(result, &tmp)
	}
