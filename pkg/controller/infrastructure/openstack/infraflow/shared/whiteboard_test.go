...
var _ = Describe("Whiteboard", func() {
	It("should create hierarchical whiteboard from flat map", func() {
		...
		Expect(w.GetChild("child1").GetChild("subchild1").Get("key1")).To(Equal(new("id111")))
		Expect(w.GetChild("child1").GetChild("subchild2").Get("key1")).To(Equal(new("id121")))
		Expect(w.GetChild("child2").IsEmpty()).To(BeFalse())
		Expect(w.GetChild("child2").Get("key2")).To(Equal(new("id22")))
		...
		Expect(generation2).To(BeNumerically(">", generation1))
		w.GetChild("child1").GetChild("subchild1").Set("key1", "id111b")
		Expect(w.CurrentGeneration()).To(BeNumerically(">", generation2))
