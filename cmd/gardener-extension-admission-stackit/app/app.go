...
const AdmissionName = "admission-stackit"

var log = logf.Log.WithName("gardener-extension-admission-stackit")

// NewAdmissionCommand creates a new command for running an stackit admission webhook.
func NewAdmissionCommand(ctx context.Context) *cobra.Command {
	var (
		...
		mgrOpts  = &controllercmd.ManagerOptions{
			...
			WebhookCertDir:          "/tmp/admission-stackit-cert",
		}
	...
	cmd := &cobra.Command{
		Use: fmt.Sprintf("admission-%s", providerstackit.Type),
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			if err := stackitinstall.AddToScheme(mgr.GetScheme()); err != nil {
